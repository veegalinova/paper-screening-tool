            ...
            {formData.include_not_include === 'include' && (
              <div className={`mb-4 p-4 rounded-lg ${!formData.db_used_in_the_study.trim() ? 'bg-red-50 border-2 border-red-400' : 'bg-green-50 border border-green-300'}`}>
                <label className="block font-semibold mb-2 flex items-center">
                  ...
                  {!formData.db_used_in_the_study.trim() && (
                ...
                <textarea
                  ...
                  className={`w-full rounded-lg px-3 py-2 ${!formData.db_used_in_the_study.trim() ? 'border-2 border-red-400 bg-white' : 'border border-green-400 bg-white'}`}
                  rows="3"
                  placeholder="⚠️ Enter dataset names separated by semicolons (REQUIRED)"
                />
              </div>
            )}
